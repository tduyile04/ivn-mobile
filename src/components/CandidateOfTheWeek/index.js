...
import {StyleSheet, Image, Alert, FlatList, Platform} from 'react-native';
    ...
    return (
      <Container>
        ...
        <Content style={styles.textViewStyle}>
                    ...
                    <Button 
                      style={Platform.OS === 'ios' ? styles.followButton : [styles.followButton, { height: 25 }]} 
                      iconLeft 
                      onPress={() => this.setState(prevState => ({ clicked: !prevState.clicked }))}>
											<Icon type="Feather" name='user-plus' style={styles.userIcon} />
                      {this.state.clicked 
                        ? <Text style={styles.followText}>{"Endorse"}</Text>
                        : <Text style={styles.followText}>{"Unendorse"}</Text>}
										</Button>
									</View>
								</View>
